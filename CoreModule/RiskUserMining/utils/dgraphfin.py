...
def degree_feat(edge_index, x):
    # 检查edge_index中的最大索引是否超出了x的第一维的大小
    max_index = np.max(edge_index)
    if max_index >= x.shape[0]:
        print(f'Error: Edge index {max_index} exceeds node features dimension {x.shape[0]}')
        raise ValueError('Edge index exceeds node features dimension')
    adj = csr_matrix(
...
def edge_timestamp_feat(edge_timestamp, edge_index, x):
    ...
    edge_timestamp_sum = np.asarray(edge_timestamp_sum)
    return (
        edge_timestamp_mean,
...
def read_dgraphfin(folder):
    ...
    names = ["dgraphfin.npz"]
    items = [np.load(folder + "/" + name, allow_pickle=True) for name in names]
    x = items[0]["x"]
