...
def main():
    parser = argparse.ArgumentParser(description="GraphSAGE on weiboGraph")
    parser.add_argument("--dataset", type=str, default="weiboGraph")
    ...
    set_seed(42)
    dataset = DGraphFin(root="./dataset", name="GraphSAGE")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_auc >= best_auc:
            ...
            preds = out[data.test_mask].squeeze()
            # 保存测试集的预测标签
            with open(model_dir + "preds_test.txt", "w") as f:
                for item in preds.tolist():
                    f.write("%s\n" % item)
            # 保存训练集和验证集的预测标签
